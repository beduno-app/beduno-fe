...
- **Decision**: FIXED — both `getByLabel(/password/i)` occurrences in `e2e/auth.spec.ts` now use `getByLabel('Hasło')`. Verified via a real pushed-branch CI run (34538929083): 24 passed, 1 failed (only `setup`, on the still-missing test-account credentials — expected), 2 did not run (`smoke.spec.ts` on both projects, correctly blocked on `setup`). All 9 pre-existing guard tests now genuinely pass.
