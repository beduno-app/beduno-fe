...
checkpoint:
  ...
  gap_pass: "Step 7 — gaps 1, 2, 3 and 5 addressed; 4 and 6 accepted as warnings"
  user_stories: 6
  gray_areas_resolved:
    - topic: "change category"
      decision: "unbounded — from complete rewrite through small fixes to new features; no part of the system is off the table"
    - topic: "why now"
      decision: "a real pilot with an agency is coming; that date is the only hard wall"
    - topic: "definition of finalized"
      decision: "all four bars accepted at once: three must-not-fail moments end-to-end, security/access control correct, every open box in implementation-plan.md closed, an operator prefers it to WhatsApp"
    - topic: "preserved behaviour"
      decision: "none — the product is pre-production and has never run a night; no backward compatibility, migration, or live integration constrains the work"
    - topic: "locked product decisions"
      decision: "nothing is locked, including propose→confirm; docs/idea.md decisions are re-openable"
    - topic: "primary persona"
      decision: "Agency Admin — the main client / buyer; Agency Planner, Property Admin, Front Desk and Worker are other roles"
    - topic: "acceptance authority"
      decision: "the team itself; no external client sign-off, so the bar must be written down or it drifts"
    - topic: "worker access"
      decision: "read-only self view — worker signs in and sees their own stay; never asserts state, so propose→confirm is intact"
    - topic: "RBAC inversion"
      decision: "make the code match the documented policy — Property Admin owns room inventory, agency roles alone create/modify/cancel planned stays"
    - topic: "audit visibility"
      decision: "widen /audit to PROPERTY_ADMIN scoped to their own property"
    - topic: "auth mechanism"
      decision: "changing — move tokens out of localStorage and make session/device revocation real"
    - topic: "scope vs timeline"
      decision: "team declined the scope-down; all twelve items stay in scope, sustained-effort cost acknowledged 2026-08-16"
    - topic: "delivery window"
      decision: "unresolved — stated as 'asap'; no week count and no pilot date"
    - topic: "secondary criteria"
      decision: "Worker self view, sub-10s check-in, E2E journeys in CI, and exports are all nice-to-have, not sufficient for success"
  product_type: web-app          # native mobile is a future intention, not this change
  target_scale:
    users: large                 # dozens of staff plus hundreds of worker accounts
    qps: low
    data_volume: small
  timeline_budget:
    delivery_weeks: 5             # working estimate, 4-6 weeks; no external commitment
    hard_deadline: null
...
- The QR checksum is an unkeyed djb2 hash: an integrity check, not
  forgery-resistant.
- `POST /stays/{id}/move` has an unresolved response shape pending a backend
  decision.
...
- **Worker** — *newly named as a role in this session.* Workers currently exist
...
toast; the interval is hard-coded and not configurable by anyone. Authorisation
is route-level only: `meta.requiresAuth` and `meta.roles` evaluated by a single
...
4. **The auth mechanism itself changes.** Tokens move out of `localStorage`, and
   ...
   overview's compliance section promises that revocation "must work instantly"
...
- The Worker read-only self view. Promoted to must-have in Phase 6, then
  demoted again in the Step 7 gap pass: FR-005 is nice-to-have and the pilot's
...
Acknowledged on 2026-08-16: the scope tallied in Phase 3 (twelve items spanning
...
The scope-down was offered with concrete cuts and explicitly declined — "we'll
manage" — and the sustained-effort cost was accepted. All twelve items remain in
scope. No further warnings are raised in this document.

Delivery window was given as "asap" during Phase 3 and quantified during the
Step 7 gap pass as a 4–6 week working estimate (`delivery_weeks: 5`). There is
still no fixed pilot date, so `hard_deadline` remains null: the estimate is
internal, not a commitment to anyone.

## Functional Requirements

Format per schema: `[Actor] can [capability]. Priority: ... Change: new | modified
| preserved`. `preserved` here means *must not regress* — the system is
pre-production, so nothing is protected for live users; these FRs exist so the
corrections in FR-001, FR-002, FR-008 and FR-020 cannot quietly damage the
operational core.

### Access Control

- FR-001: Property Admin can manage room inventory for their own property. Priority: must-have. Change: modified
  > Socrates: Capacity is commercial, not operational — room count and capacity are contract terms belonging to whoever signed the contract, rather than to the on-site manager.
  > Resolution: pending.
- FR-002: Only agency roles can create, modify or cancel a planned stay. Priority: must-have. Change: modified
  > Socrates: Exclusive agency ownership makes the agency a bottleneck at 8pm: a property with a free bed and a worker at the door cannot act. Directly threatens the "front desk falls back to WhatsApp" guardrail.
  > Resolution: pending.
- FR-003: Property Admin can view the audit log for their own property. Priority: must-have. Change: modified
  > Socrates: Considered: scoping is harder than a route gate; partners are external parties; the buyer did not ask for it.
  > Resolution: kept as written — dispute resolution is worthless if only one side sees the evidence.
- FR-004: Agency Admin can view the audit log across all properties. Priority: must-have. Change: preserved
  > Socrates: Considered: cross-property visibility is the asymmetry partners distrust, and exceeds what any single dispute needs.
  > Resolution: kept as written — the agency is accountable for its workers across every property.
- FR-005: Worker can sign in and view their own current stay. Priority: nice-to-have. Change: new
  > Socrates: A worker self view puts worker PII on unmanaged personal phones — a new GDPR surface in a product whose compliance story is "store the minimum".
  > Resolution: pending.
- FR-006: Agency Admin can create, edit and deactivate user accounts. Priority: nice-to-have. Change: new
  > Socrates: Deactivation without revocation is half a feature: an account deactivated while its token stays valid until expiry is not deactivated. Meaningless unless FR-007 lands with it.
  > Resolution: pending.
- FR-007: Agency Admin can revoke an active session or device. Priority: nice-to-have. Change: new
  > Socrates: Revocation cannot reach an offline device. A revoked BYOD phone keeps working from its cached snapshot, so "instant" revocation is false on exactly the devices it exists to protect.
  > Resolution: pending.
- FR-008: A signed-in user's tokens are not readable from `localStorage`. Priority: must-have. Change: modified
  > Socrates: A stricter token store collides with offline-first: httpOnly cookies or in-memory tokens need a reachable refresh endpoint that the offline front desk does not have, risking a 6am logout.
  > Resolution: pending.
- FR-009: An idle session ends automatically after a warning. Priority: must-have. Change: preserved
  > Socrates: Considered: 30 minutes is wrong for a night shift; the timeout is hard-coded though documented as configurable; logout with a non-empty queue risks data loss.
  > Resolution: kept as written — shared BYOD devices need an idle timeout.

### Arrival Day

- FR-010: Front Desk can see today's expected arrivals for their property. Priority: must-have. Change: preserved
  > Socrates: "Today" is ambiguous on a night shift: a worker arriving at 01:00 is tomorrow by date and tonight by shift, so the arrivals list silently drops them and nobody notices until the bed is empty.
  > Resolution: pending.
- FR-011: Front Desk can check a worker in by QR scan or manual search. Priority: must-have. Change: preserved
  > Socrates: Considered: the QR checksum is forgeable; camera access on BYOD is not guaranteed; manual search exposes the worker directory.
  > Resolution: kept as written — two independent paths to the same action is correct design.
- FR-012: Front Desk can mark a no-show with a localized reason code. Priority: must-have. Change: preserved
  > Socrates: The undo path is undefined: a worker marked no-show at 20:00 who turns up at 23:00 leaves the stay in NO_SHOW with no documented route back to CHECKED_IN, making the nightly count wrong.
  > Resolution: pending.

### Nightly List

- FR-013: Front Desk can view the in-house roster per room. Priority: must-have. Change: preserved
  > Socrates: Considered: in-house is today-only so midnight splits the roster; the screen shows belief rather than verified reality; names are readable on an unlocked phone.
  > Resolution: kept as written — the nightly list is the product's core claim about itself.
- FR-014: Front Desk can check a worker out. Priority: must-have. Change: preserved
  > Socrates: Considered: departures are rarely observed so occupancy drifts; planned and early exits are indistinguishable; per-worker checkout is what makes FR-015 necessary.
  > Resolution: kept as written — physical confirmation of departure is the property side owning reality.
- FR-015: Front Desk can check out multiple workers in one action. Priority: nice-to-have. Change: new
  > Socrates: Considered: one mis-tap empties an occupied room; bulk flattens per-worker audit granularity; it stands in for an unmodelled contract-ending event.
  > Resolution: kept as written — twenty workers on one bus is the normal case.
- FR-016: Front Desk can move a worker between rooms or properties. Priority: must-have. Change: modified
  > Socrates: Considered: the response shape is unresolved so acceptance criteria cannot be written; a cross-property move writes outside the actor's scope; MOVED plus a new stay is two rows per human.
  > Resolution: kept as written — but remains blocked on the beduno-be decision, see Open Questions.

### Inspection Day

- FR-017: Front Desk can complete a room-by-room inspection walkthrough. Priority: must-have. Change: preserved
  > Socrates: Considered: inspection is absent from the offline queue; capturing a discrepancy is not resolving one; losing the session loses the walk.
  > Resolution: kept as written — Inspection Day is the third must-not-fail moment.

### Offline

- FR-018: Operational actions taken offline are queued and replayed on reconnect. Priority: must-have. Change: preserved
  > Socrates: Replay assumes the server state did not move: a queued action carries the stayId, not the state it was valid against, so it can write into a world that no longer exists.
  > Resolution: pending.
- FR-019: Conflicts on replay land in a review inbox, never auto-merged. Priority: must-have. Change: preserved
  > Socrates: Nobody owns the inbox. "Needs review" with no assignee, no SLA and no escalation is a queue that only grows; the design correctly refuses to auto-merge and then leaves the decision homeless.
  > Resolution: pending.
- FR-020: The offline snapshot carries only fields the ops screens render. Priority: must-have. Change: modified
  > Socrates: "What the screens render" is a moving target defined by current UI rather than by policy, so any new field on any ops screen silently widens the snapshot again.
  > Resolution: pending.

### Planning & Reporting

- FR-021: Agency Planner can create planned stays, singly and in bulk. Priority: must-have. Change: preserved
  > Socrates: Considered: bulk assign rubber-stamps a machine's placement; plans are built on stale occupancy; it concentrates the bottleneck FR-002 creates.
  > Resolution: kept as written — one-at-a-time planning is the spreadsheet workflow the product replaces.
- FR-022: Conflict engine blocks hard violations, allows soft ones with override. Priority: must-have. Change: preserved
  > Socrates: Routine overrides are not a control: a soft warning that appears on most assignments is clicked through reflexively, giving gender and over-planning rules the appearance of governance without the effect.
  > Resolution: pending.
- FR-023: Agency Admin can export nightly occupancy and exception reports. Priority: nice-to-have. Change: preserved
  > Socrates: Export is an uncontrolled PII egress point: a CSV of who sleeps where leaves the system's access controls entirely and lands in email and WhatsApp — the channels the product exists to replace.
  > Resolution: pending.
...
This change **modifies** the existing rule, and the Step 7 gap pass settled why.
Phase 5 first named the rule as admissibility alone, which left "modifies"
...
product believes about who owns reality, which Phase 1 established is open.
...
rollback plan to write, because nothing has ever run. The brownfield
`## Constraints & Compatibility` section of the resulting PRD will be
substantially empty, and should say so plainly rather than inventing constraints
to fill it.
...
> guardrail. Resolving it may re-open propose→confirm, which Phase 1 established
...
- **No bed-level modelling.** Confirmed in the Step 7 gap pass. Workers occupy a
  spot in a room; there is no bed entity. This closes a data-model change that
...
Captured for the downstream stack-assessment step; not part of the PRD.

- Product type is a web application today. **Native mobile is intended for the
  future** but is not this change. Worth carrying forward, because an offline
  PWA and a native app make different demands of the token-storage decision in
  Cluster A.

## Quality cross-check

Run at Step 7 on 2026-08-16. All six required elements are present. Four gaps
were addressed in a gap pass; two were accepted as warnings and are carried here
for `/10x-prd` to mirror into `## Open Questions`.

**Addressed**

- *Business Logic classification* — resolved: authority is part of the domain
  rule, so the change genuinely modifies it.
- *Delivery window* — resolved: `delivery_weeks: 5` (4–6 week working estimate).
  A pilot date still does not exist.
- *Declined exclusions* — resolved: bed-level modelling is out. Job-site and
  shift linkage was confirmed **in scope**, which is a scope addition, not a
  closure.
- *FR prioritisation* — resolved: 18 must-have, 5 nice-to-have (FR-005, FR-006,
  FR-007, FR-015, FR-023 demoted). A cut line now exists.

**Accepted as warnings**

- **Nothing is genuinely preserved.** Correct for a pre-production system, but it
  means the 13 `preserved`-tagged FRs are intentions about not regressing rather
  than protections. Nothing external catches it if they break — and the E2E
  suite that would is itself Secondary.
- **Thirteen of 23 FRs carry `Resolution: pending`, and nine open questions are
  `Block: yes`.** The largest single body of unresolved work in these notes is
  design decisions, not implementation. Clusters A–E name them; each needs one
  decision rather than several local fixes.

**One further warning surfaced by the gap pass itself**

- **Job-site and shift linkage entered scope by omission.** It has no FR, no user
  story, no challenge and no cost estimate, and the 4–6 week window was set
  without it. It is the least-specified and potentially largest item in the
  change.

## Open Questions

1. ~~**What does the Worker role actually do?**~~ — *Resolved in Phase 2:*
...
9. ~~**What is the delivery window in weeks?**~~ — *Resolved in the Step 7 gap
   pass:* 4–6 weeks, recorded as `delivery_weeks: 5`. **Still open:** there is no
   fixed pilot date, so `hard_deadline` is null and the forcing function Phase 1
...
14. **Cluster C — the permission model has unowned edges.** FR-001 (capacity is
    ...
    propose→confirm, which Phase 1 established is not locked.
...
16. **Cluster E — soft constraints may be governance theatre.** FR-022: an
    ...
    Owner: team. Block: no.
17. ~~**Does the domain rule actually change?**~~ — *Resolved in the Step 7 gap
...
19. ~~**Is bed-level modelling in or out?**~~ — *Resolved in the Step 7 gap
...
21. **How does a worker authenticate?** — Promoted to blocking in Phase 6, then
    de-escalated in the Step 7 gap pass when FR-005 returned to nice-to-have.
